...
from vpt_plugin_watershed.stardist.seeds import StardistSeedsExtractor
...
def test_run_watershed() -> None:
    ...
    parameters = {"stardist_model": TEST_MODEL, "seed_channel": "DAPI", "entity_fill_channel": "PolyT"}

    nucs, cells = run_watershed(images, parameters, ["nuclei", "cell"])
    assert len(nucs[0]) == 1
    assert len(nucs[1]) == 1
    assert cells[0, 128, 128] == 1
    assert cells[1, 128, 128] == 1

    nucs, cells = run_watershed(images, parameters, ["cell"])
    assert nucs is None
...
def test_stardist_interop() -> None:
    ...
    seg_result = polygons_from_stardist(
        [result], {"simplification_tol": 2, "smoothing_radius": 3, "minimum_final_area": 100}
    )
